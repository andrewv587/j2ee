package test.com.bjsxt.hibernate;
...
import com.bjsxt.hibernate.Msg;
import com.bjsxt.hibernate.MsgInfo;
import com.bjsxt.hibernate.Topic;
...
public class HibernateQLTest {
	...
	public void testHQL_02() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("from Category c where c.name > 'c2'");
		List<Category> categories = (List<Category>)q.list();
		for(Category c : categories) {
			System.out.println(c.getName());
		}
...
//		q.setParameter(1, 8);
	...
	public void testHQL_10() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("from Topic t where t.category.name = 'c1'");
		List<Topic> topics = (List<Topic>)q.list();
		for(Topic t : topics) {
			System.out.println(t.getTitle());
		}
