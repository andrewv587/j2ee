...
<html>
<%
List<Manager> manager_coll=(List<Manager>)session.getAttribute("managerQuery");
...
if(manager_coll==null || manager_coll.isEmpty()){
%>
  ...
  <tr>
    <td>
      <a href="manager_managergoAdd">添加管理员信息</a> </td>
  </tr>
</table>
 <%
}else{
  //通过迭代方式显示数据
  Iterator it=manager_coll.iterator();
  int ID=0;
  String name="";
  int sysset=0;
  int readerset=0;
...
<%
  while(it.hasNext()){
    ...
    Manager manager_itr=(Manager)it.next();
	ID=manager_itr.getId();
	name=manager_itr.getName();
	Purview manager_purview=manager_itr.getPurview();
	sysset=manager_purview.getSysset();
	readerset=manager_purview.getReaderset();
	bookset=manager_purview.getBookset();
	borrowback=manager_purview.getBorrowback();
	sysquery=manager_purview.getSysquery();
	%> 
  <tr>
    ...
    <td align="center"><%if(!name.equals("tsoft")){ %><a href="manager_managerModifyQuery?id=<%=ID%>">权限设置</a><%}else{%>&nbsp;<%}%></td>
    <td align="center">
	<%if(!name.equals("tsoft")){ %><a href="manager_managerDel?id=<%=ID%>">删除</a><%}else{%>&nbsp;<%}%></td>
  </tr>
